...
class Verifier:
    ...
    def expect_equal(self, label: str, actual: int | str | set[str], expected: int | str | set[str]) -> None:
        if actual == expected:
    ...
    def verify_c10(self) -> None:
        root = self.dataset_root / "c10" / "cifar-10-batches-py"
        ...
        if all((root / name).is_file() for name in official_files):
            train_count = 0
            sample_width = None
        ...
        self.verify_imagefolder_layout(root, "CIFAR-10", 10, 50000, 10000)

    def verify_c100(self) -> None:
        root = self.dataset_root / "c100" / "cifar-100-python"
        if not self.require_dir(root):
            return

        official_files = ["train", "test", "meta"]
        if all((root / name).is_file() for name in official_files):
        ...
        self.verify_imagefolder_layout(root, "CIFAR-100", 100, 50000, 10000)


    def verify_imagefolder_layout(
        self,
        root: Path,
        display_name: str,
        ...
        test_root = root / "test"
        if not self.require_dir(train_root) or not self.require_dir(test_root):
            return

        train_classes = sorted(path for path in train_root.iterdir() if path.is_dir())
        test_classes = sorted(path for path in test_root.iterdir() if path.is_dir())
        train_images = self.count_pngs(train_root)
        test_images = self.count_pngs(test_root)

        self.expect_equal(f"{display_name} train classes", len(train_classes), expected_classes)
        self.expect_equal(f"{display_name} test classes", len(test_classes), expected_classes)
        self.expect_equal(f"{display_name} train images", train_images, expected_train)
        self.expect_equal(f"{display_name} test images", test_images, expected_test)

    def verify_s10(self) -> None:
        root = self.dataset_root / "s10" / "stl10_binary"
        if not self.require_dir(root):
            return
        ...
        self.expect_equal("STL10 train images", train_images, 5000)
        self.expect_equal("STL10 train labels", train_labels, 5000)
        self.expect_equal("STL10 test images", test_images, 8000)
        self.expect_equal("STL10 test labels", test_labels, 8000)
        self.expect_equal("STL10 unlabeled images", unlabeled_images, 100000)
        self.expect_equal("STL10 classes", len(classes), 10)

    def verify_t200(self) -> None:
        root = self.dataset_root / "t200"
        if not self.require_dir(root):
            return
        ...
        required_dirs = [train_root, val_root, val2_root, test_images_root]
