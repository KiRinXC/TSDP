...
`head_only` 是分类头贡献控制组，不接受 layer、unit 或 scalar 选择参数。当前 `ResNet18+CIFAR-100` 固定保护 `last_linear.weight` 和 `last_linear.bias`，即 unit `120-121`；其余 `0-119` 个 backbone unit 全部从 victim `best.pth` 复制。该组保护 `51,300/11,227,812` 个参数，参数比例为 `0.4569%`，mask SHA256 固定为 `466d73220c084463772303efcfa567e3326b91f04ee04ed15d9db8f6c3b46785`，分类头使用 `replace`，随后按统一协议对全部参数共同微调。它回答“只隐藏分类头本身能够抑制多少 MS”，不重新比较 Lab 02 已经确定的 replace/adapter 选择。
...
`head_only` 使用普通 `ResNet18+CIFAR-100` victim，与完整层、大权重和 TensorShield 共享同一查询与攻击协议。正式运行命令为：

```bash
bash exp/MS/train_surrogate/run.sh resnet18 c100 \
  --defense head_only --budget 500 \
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" exp/MS/train_surrogate/plan.py
```
