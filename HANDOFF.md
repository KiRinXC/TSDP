...
- `lab/04_tensorshield`：作者 eligible rank 的 Top-1 至 Top-17 前缀曲线、Top-12 完整 leave-one-out 与五组联合删除，以及排除分类头后的候选位置集合对照。消融所有 case 均固定保护分类头 bias；删除 rank-3 分类头 weight 后为 `mixed`，其余为 `replace`。单删结果表明条件贡献高度不均匀：rank-3 影响最大；卷积项中 rank-9/rank-12 反弹最大；rank-5/rank-10 单删反而使三项攻击指标一致变弱；rank-6/7/8 指标方向混合。删除 5/10 以 `23.1115%` 参数得到 `0.1579/0.1705/2.718431`，在 accuracy/fidelity 上最接近黑盒。以删除 5/8/10 为基准的 rank-6/unit-36 与 rank-7/unit-48 完整 2×2 表明：只暴露上游 unit 36 时 accuracy/fidelity 几乎不变，只暴露下游 unit 48 时反弹 `0.54/1.01` 个百分点；两者同时暴露时达到 `0.1762/0.1936/2.608998`，交互项为 `+0.85/+0.92/-0.036426`，支持“下游主要割点 + 上游条件节点”的串行图解释。位置集合的 `first_10` 与主曲线 Top-11 三指标逐值相同；`spread_10` 为 `0.2121/0.2342/2.419835`，明显弱于 `first_10`。
