...
训练普通 surrogate 的 `lab/02`、`04`、`05`、`06` 已统一复用
...
- `lab/04_tensorshield`：作者 eligible rank 的 Top-1 至 Top-17 前缀曲线、Top-12
  完整 leave-one-out 与五组联合删除、三策略十种子候选，以及前/后/分散十项位置
  ...
  十种子实验保留三根柱子：Top-10、Top-10+全部 BN gamma、删除 rank-5/8/10
  +BN gamma；三组均保护分类头 weight/bias。三个均值依次为
  `0.17277/0.18638/2.66467`、`0.13136/0.13994/3.01441` 和
  `0.11975/0.12755/3.13450`。BN gamma 相对 Top-10 在 `10/10` seed 上三指标
  同向改善；给定 BN gamma 后删除 rank-5/8/10 也在 `10/10` seed 上未引发攻击
  反弹，并将比例从 `9.0362%` 降到 `7.0662%`。相对逐 seed soft 黑盒，三项同时
  达到的次数依次为 `0/10`、`8/10`、`9/10`；候选唯一例外 seed 48 仅 KL
  低于黑盒 `0.018050`。该集合仍为读取 seed-42 MS 反馈的后验有效集合，不是
  先验方法；暴露更多信息后的指标改善只能解释为固定攻击优化负迁移。
