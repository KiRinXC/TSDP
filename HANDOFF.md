...
因此只执行 `git clone` 不能继续实验。本次迁移包应位于项目父目录的 `TSDP.tar.gz`，并包含上述数据、权重、当前结果和 `.git` 历史，确保最新本地提交可以完整迁移；为减小体积，迁移包只排除 `.codex`、`.ipynb_checkpoints`、`__pycache__` 和 `*.pyc`。解压后按 `requirements.lock.txt` 恢复唯一环境，再依次运行 `make gpu`、`make unit` 和 `make verify`。
