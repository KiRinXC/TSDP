...
eval_ms        官方 test 或 validation split，只用于最终评估
```

query 因而可能与 victim 训练数据重叠，这是当前论文 baseline 协议的有意选择，不是数据泄漏 bug。随机种子统一为 `42`。
...
- `lab/03_baseline`：四种通用 baseline 的保护比例曲线。
...
7. 与浅层、中间层、深层、大权重和 TensorShield 在相近实际成本处比较。
...
TEESlice 必须使用不同标记并注明 victim 结构不同，不能与普通 victim 曲线连成同一条成本曲线。
...
当前应通过 32 个测试。运行后清理任何意外生成的 `__pycache__` 和 `*.pyc`。
...
Git 只保存代码、文档和小型结果；以下实验必需资产被 `.gitignore` 排除：
...
因此只执行 `git clone` 不能继续实验。本次迁移包应位于项目父目录的 `TSDP.tar.gz`，并包含上述数据、权重、当前结果和 `.git` 历史，确保最新本地提交可以完整迁移；为减小体积，迁移包只排除 `.codex`、`.ipynb_checkpoints`、`__pycache__` 和 `*.pyc`。解压后先运行第 8 节测试，再核对 GPU、PyTorch、torchvision 和数据路径。

当前项目总大小约 15G，其中公开数据约 6.8G、权重约 7.4G。压缩包校验值保存在同目录的 `TSDP.tar.gz.sha256`。
