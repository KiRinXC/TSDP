...
三张图统一使用 `protected_state_byte_ratio` 作为横坐标，并采用对数刻度。各保护组只绘制独立散点，不把相互重叠的语义组连接成曲线；无保护和全保护使用当前正式协议的 `end` 指标作为水平参考线。
...
若现有 `metrics.json`、`history.tsv` 和 mask 与当前协议及语义定义完全一致，可复用已经完成的保护组，只训练缺失组：
...
复用入口会重新构造并核对每组 mask 摘要、参数统计、checkpoint、posterior 和训练协议；不一致时直接拒绝复用。

## 输出

```text
results/lab/05_state/metrics.json               十八组协议、保护统计和原始指标
results/lab/05_state/history.tsv                十八组各 100 轮训练与评估记录
