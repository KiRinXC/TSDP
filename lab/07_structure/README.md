...
`results/lab/07_structure/tensorshield_top17.tsv`，同时保留每个 weight 的原始
...
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/07_structure/run.py
```

核对对象为：

```text
results/lab/07_structure/resnet18_c100.tsv
```
...
`dependency.py` 固定 Lab04 十随机种子实验得到的 5.7529% 基础保护集合：eligible
...
一次。基础集合和同 seed soft full-protection 黑盒直接读取 Lab04 相同协议结果，
不重复训练。
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/07_structure/dependency.py --dry-run
```
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/07_structure/dependency.py
```
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/07_structure/dependency.py --resume
```

输出为：

```text
results/lab/07_structure/dependency.json
results/lab/07_structure/dependency.tsv
results/lab/07_structure/dependency_history.tsv
results/lab/07_structure/dependency.png
results/lab/07_structure/dependency_*_mask.pt
```
...
conv1 组直接复用 Lab04/Lab07 的十种子结果，保护 645,924 个参数，占 5.7529%。
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/07_structure/swap.py --dry-run
```
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/07_structure/swap.py --resume
```

输出为：

```text
results/lab/07_structure/swap.json
results/lab/07_structure/swap.tsv
results/lab/07_structure/swap_history.tsv
