...
本实验保留原候选图的三根柱子，并把每根柱子从 seed-42 单点改为十个独立 seed
的均值与样本标准差：
...
三组都完整保护 `last_linear.weight` 与 `last_linear.bias`。第一组已经固定包含分类头
bias；第一组与第二组的唯一差异是 20 个 BN gamma，用于检验 BN gamma 的跨 seed
作用。第二组与第三组的唯一差异是暴露 rank-5/8/10，用于检验删点后是否仍能保持
保护效果。soft full-protection 黑盒按相同 seed 和 query 划分训练，只作为图中的
参考线，不作为第四根柱子。
...
每个 seed 都训练三种策略与同 seed 的 soft full-protection 黑盒对照。四组使用同一
...
并计算 `+BN gamma - Top-10`、`candidate - Top-10+BN gamma` 和
`candidate - black-box` 三组配对差值。判定某策略在一个 seed 达到黑盒要求：
