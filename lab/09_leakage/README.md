...
对同一批已泄露状态的利用强度。系统保护集合始终固定为 Lab06 最终的 5.7529%
...
public 值，避免制造没有数值含义的整数插值。0% 和 100% 端点必须分别与 Lab06
...
0% 和 100% 的训练与最终指标直接复用 Lab06 同 seed、同协议结果；只新增
...
本实验只验证混合初始化是否造成负迁移，不把该现象进一步解释为某种固定的位置机制。
...
```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" \
  lab/09_leakage/run.py --resume
```

输出固定为：

```text
results/lab/09_leakage/metrics.json
results/lab/09_leakage/data.tsv
results/lab/09_leakage/history.tsv
results/lab/09_leakage/probe.tsv
results/lab/09_leakage/metrics.png
```
