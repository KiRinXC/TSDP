...
if str(LAB06_ROOT) not in sys.path:
    sys.path.insert(0, str(LAB06_ROOT))
import candidate as closure  # noqa: E402
...
prefix = closure.prefix
EXPERIMENT = "09_leakage"
SOURCE_BASE_CASE = closure.CANDIDATE_DROP06_CASE
SOURCE_BLACKBOX_CASE = closure.BLACKBOX_CASE
EVALUATION_SEEDS = closure.EVALUATION_SEEDS
STRENGTHS = (0.0, 0.25, 0.50, 0.75, 1.0)
TRAINED_STRENGTHS = (0.25, 0.50, 0.75)
METRICS = ("surrogate_acc", "fidelity", "posterior_kl")
HISTORY_FIELDS = closure.HISTORY_FIELDS
PROBE_FIELDS = (
    "seed",
    "case",
...
def build_endpoint_models(
    ...
    public_model, blackbox_plan, full_masks = closure.initialize_blackbox(
        victim,
        official_weight,
        seed,
    )
    (
        ...
        selected_weights,
        bn_gamma,
    ) = closure.initialize_strategy(
        closure.STRATEGY_BY_NAME[SOURCE_BASE_CASE],
        victim,
        official_weight,
        seed,
    )
    if (
        ...
        raise RuntimeError("Lab06 5.7529% 基础保护集合已经变化。")
    if (
        blackbox_plan.protected_unit_count,
        blackbox_plan.protected_param_count,
    ) != (122, 11_227_812):
        raise RuntimeError("Lab06 matched soft 黑盒计划已经变化。")
    ...
    for name, mask in base_masks.items():
        if bool(mask.any()) != bool(mask.all()):
            raise RuntimeError("Lab09 只接受完整 tensor 保护集合。")
...
def load_source(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("Lab06 candidate 不是当前十种子 validation-best 来源。")
    source_by_key = {
    ...
    if set(source_by_key) != expected:
        raise ValueError("Lab06 candidate 缺少 0%/100% 端点。")
    for seed in EVALUATION_SEEDS:
        ...
        for field in (
            ...
            if source_partition[field] != current_partition[field]:
                raise ValueError(f"seed {seed} 的 query 划分与 Lab06 不一致。")
    ...
    for (seed, source_case), result in source_by_key.items():
        ...
        if len(matches) != 1:
            raise ValueError(f"Lab06 {seed}/{source_case} 缺少选中 epoch 历史。")
...
def normalize_source_result(
    ...
    *,
    ...
    return {
        ...
        "utilization_strength": strength,
        "origin": (
            "reused_lab06_matched_blackbox"
            if strength == 0.0
            else "reused_lab06_hybrid"
        ),
...
def trained_result(
    *,
    ...
    return {
        ...
        "case": case_name(strength),
        "utilization_strength": strength,
        "origin": "trained_lab09_intermediate",
        "source_case": None,
...
def load_progress(
    ...
    *,
    ...
    if not progress_path.is_file() or not history_path.is_file():
        raise FileNotFoundError("Lab09 --resume 缺少完整进度文件。")
    ...
    if (
        ...
        raise ValueError("Lab09 进度与当前来源或协议不一致。")
    results = payload.get("results")
    if not isinstance(results, list):
        raise ValueError("Lab09 进度结果格式无效。")
    ...
    if len(keys) != len(set(keys)) or not set(keys) <= expected:
        raise ValueError("Lab09 进度包含重复或未知 case。")
    ...
    if set(grouped) != set(keys) or any(count != prefix.EPOCHS for count in grouped.values()):
        raise ValueError("Lab09 进度 history 与已完成 case 不一致。")
...
def main() -> int:
    ...
    source_metrics = ROOT / "results" / "lab" / "06_weight" / "candidate.json"
    source_history = (
        ROOT / "results" / "lab" / "06_weight" / "candidate_history.tsv"
    )
    victim_checkpoint = (
    ...
    public_model, _, _ = closure.initialize_blackbox(
        victim,
        official_weight,
        EVALUATION_SEEDS[0],
    )
    prefix.configure_reproducibility(EVALUATION_SEEDS[0], deterministic=True)
    hybrid_model = closure.initialize_strategy(
        closure.STRATEGY_BY_NAME[SOURCE_BASE_CASE],
        victim,
        official_weight,
        EVALUATION_SEEDS[0],
    ...
    if args.resume:
        ...
        if progress_path.exists() or progress_history_path.exists():
            raise FileExistsError("Lab09 存在进度文件；请使用 --resume 或先核对后清理。")
    ...
    if completed != expected_trained_keys:
        raise RuntimeError("Lab09 三十个中间强度结果不完整。")
    history_rows.sort(
        key=lambda row: (
            int(row["seed"]),
    ...
    if len(history_rows) != len(expected_trained_keys) * prefix.EPOCHS:
        raise RuntimeError("Lab09 中间强度 history 不完整。")
    ...
    if set(result_by_key) != expected_keys:
        raise RuntimeError("Lab09 五强度 × 十 seed 结果不完整。")
    results = [
    ...
    if set(probe_by_key) != expected_keys:
        raise RuntimeError("Lab09 五强度 × 十 seed epoch-0 探针不完整。")
    probes = [
    ...
    payload = {
        "schema_version": 3,
        "experiment": "09_leakage_utilization",
        "protocol": "MS",
        **prefix.protocol_metadata(first_query),
        ...
        "source": {
            "lab06_candidate": str(source_metrics.relative_to(ROOT)),
            "lab06_candidate_sha256": source_sha256,
            "lab06_history": str(source_history.relative_to(ROOT)),
            "lab06_history_sha256": source_history_sha256,
