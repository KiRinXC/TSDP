...
EXPERIMENT = "07_bn_drop"
OUT_DIR = ROOT / "results" / "lab" / "07_bn"
SOURCE_PATH = ROOT / "results" / "lab" / "06_weight" / "candidate.json"
SOURCE_HISTORY_PATH = (
    ROOT / "results" / "lab" / "06_weight" / "candidate_history.tsv"
)
RESULT_PATH = OUT_DIR / "drop.json"
DATA_PATH = OUT_DIR / "drop.tsv"
HISTORY_PATH = OUT_DIR / "drop_history.tsv"
PLOT_PATH = OUT_DIR / "drop.png"
EVALUATION_SEEDS = tuple(range(43, 53))
SOURCE_ALL_CASE = closure.CANDIDATE_DROP06_CASE
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="复用 drop.json 中与当前协议、seed 和 mask 一致的已完成训练 case。",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def derive_gamma_groups(victim: nn.Module) -> dict[str, tuple[str, ...]]:
    all_gamma = closure.derive_bn_gamma(victim)
    groups = {
...
def load_source(
    ...
    if not SOURCE_PATH.is_file() or not SOURCE_HISTORY_PATH.is_file():
        raise FileNotFoundError("缺少 Lab06 candidate 十种子来源。")
    ...
    for field, value in expected.items():
        if payload.get(field) != value:
            raise ValueError(f"Lab06 candidate 的 {field} 与当前实验不一致。")
    if tuple(payload.get("evaluation_seeds", ())) != EVALUATION_SEEDS:
        raise ValueError("Lab06 candidate 的十个 seed 与当前实验不一致。")
    ...
    if tuple(sorted(all_rows)) != EVALUATION_SEEDS or tuple(sorted(blackbox_rows)) != EVALUATION_SEEDS:
        raise ValueError("Lab06 candidate 缺少 all-gamma 或 matched soft 黑盒十种子结果。")
    ...
    for seed in EVALUATION_SEEDS:
        ...
        if actual_states != expected_states:
            raise ValueError(f"Lab06 seed {seed} 的 all-gamma mask 与当前实验不一致。")
        if row["protection"]["protection_mask_sha256"] != expected_mask_sha256:
            raise ValueError(f"Lab06 seed {seed} 的 all-gamma mask 哈希不一致。")
        for candidate_row in (row, blackbox_rows[seed]):
            if candidate_row["query_partition_seed"] != seed:
                raise ValueError(f"Lab06 seed {seed} 的 query 划分元数据错误。")
            if candidate_row["result"].get("eval_passes") != 1:
                raise ValueError(f"Lab06 seed {seed} 不是单次 eval_ms 结果。")
        ...
        for field in (
            ...
            if source_partition[field] != current_partition[field]:
                raise ValueError(f"Lab06 seed {seed} 的 {field} 与当前划分不一致。")
    ...
    if len(source_history) != len(EVALUATION_SEEDS) * prefix.EPOCHS:
        raise ValueError("Lab06 all-gamma history 不完整。")
...
def load_resume_rows(
    *,
    ...
    if (
        ...
        raise ValueError("现有 drop.json 不能在当前协议下 resume。")
    retained = []
...
def main() -> int:
    ...
    print(
        f"[SOURCE] 复用 Lab06 {len(all_source)} 个 all-gamma 与 "
        f"{len(blackbox_by_seed)} 个 matched soft 黑盒结果。",
        flush=True,
    )
    if args.dry_run:
        print("[INFO] dry-run 完成，未写入 Lab07 BN 产物。")
        return 0

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    mask_paths = {
        case: OUT_DIR / f"drop_{case.removeprefix('drop_')}_mask.pt"
        for case in CASES
    }
    for case in CASES:
    ...
    payload = {
        ...
        "protocol": "MS_bn_gamma_group_drop",
        "attack_protocol": prefix.ATTACK_PROTOCOL_VERSION,
        "dataset": prefix.DATASET,
        "victim_model": prefix.MODEL,
        "query_budget": prefix.BUDGET,
