...
prefix = base.prefix
EXPERIMENT = "07_bn_add"
OUT_DIR = ROOT / "results" / "lab" / "07_bn"
RESULT_PATH = OUT_DIR / "add.json"
DATA_PATH = OUT_DIR / "add.tsv"
HISTORY_PATH = OUT_DIR / "add_history.tsv"
PLOT_PATH = OUT_DIR / "add.png"
SEED = 42
...
def main() -> int:
    ...
    for case in CASES:
        ...
        del surrogate
    if args.dry_run:
        print("[INFO] dry-run 完成，未写入 Lab07 BN add 产物。")
        return 0
    ...
    mask_paths = {case: OUT_DIR / f"add_{case}_mask.pt" for case in CASES}
    for case in CASES:
