...
`candidate.py` 在上述 seed-42 闭包扫描之后固定四种策略：TensorShield Top-10、
Top-10+全部 BN gamma、删除 rank-5/8/10+BN gamma，以及额外删除 rank-6 的
最终 5.7529% 候选。候选选择使用 seed 42；稳定性验证预先固定未参与选点的
seed 43–52，并为每个 seed 训练四种策略与 matched soft 黑盒。

每个 case 都独立重放 canonical 初始化和同 seed 的 400/100 query 划分，最多训练
100 epoch，按 validation soft cross-entropy 选择最早 best，固定后只评估一次
`eval_ms`。该结果是后验科学验证，不是无偏先验选择器。

```bash
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" lab/06_weight/candidate.py --dry-run
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" lab/06_weight/candidate.py
"$HOME/venvs/dl-py310-torch210-cu121/bin/python" lab/06_weight/candidate.py --resume
```

```text
results/lab/06_weight/candidate.json
results/lab/06_weight/candidate.tsv
results/lab/06_weight/candidate_history.tsv
results/lab/06_weight/candidate.png
results/lab/06_weight/candidate_*_mask.pt
```
