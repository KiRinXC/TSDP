...
当前主黑盒边界固定为普通 victim 的 soft-posterior `full_protection`。Hard-label `hard_blackbox` 的查询输出能力不同，不加入本实验主图，也不用于选择组合。
...
若已有部分结果与当前协议、输入哈希、mask 和完整 100 轮历史完全一致，可复用已完成组合并继续训练缺失组合：
...
复用入口允许在稳定协议上新增彼此独立的受控曲线，但只复用定义仍然存在的完整 case。每个可复用 case 都会重新核对输入哈希、作者 rank、保护集合、mask、训练参数和完整 100 轮历史；任一项不一致时直接拒绝复用。

## 输出

```text
results/lab/06_weight/metrics.json       48 个组合、输入哈希、保护统计与 end 指标
results/lab/06_weight/history.tsv        40 个新增组合共 4,000 轮 query 训练记录
...
横轴使用 `Top-k`，便于在相同作者前缀下比较新增语义；每个点的实际保护参数数量与比例保存在 `data.tsv` 和 `metrics.json`。主图只绘制当前研究问题直接使用的 soft full-protection 黑盒水平线，以免 no-protection 白盒线压缩关键区间；两条边界的原始数值仍完整保存在 `metrics.json`。
