...
sys.path.remove(str(LAB06_ROOT))

prefix = closure.prefix
EXPERIMENT = "08_structure"
BASE_CASE = "five_conv1_bn_gamma_head"
BLACKBOX_CASE = closure.BLACKBOX_CASE
...
def build_selected_states(
    ...
    base_states, base_weights, bn_gamma = closure.build_candidate_drop06(victim)
    ...
    if base_weights != expected_weights:
        raise ValueError("Lab06 的 5.7529% 基础集合已经变化。")
    conv1_weights = tuple(
...
def load_source(
    ...
    *,
    ...
    payload = closure.load_json(path, "Lab06 candidate")
    if (
        tuple(payload.get("evaluation_seeds", ())) != EVALUATION_SEEDS
        ...
        raise ValueError("Lab06 candidate 与当前 victim、posterior 或 seeds 不一致。")
    source_cases = {
        closure.CANDIDATE_DROP06_CASE,
        BLACKBOX_CASE,
    }
    result_by_key = {
    ...
    if set(result_by_key) != expected:
        raise ValueError("Lab06 candidate 缺少基础集合或 matched 黑盒结果。")
    for seed in EVALUATION_SEEDS:
...
def trained_result(
    *,
    ...
    return {
        "seed": seed,
        "case": spec.name,
        "origin": "trained_lab08_leave_one_out",
...
def main() -> int:
    ...
    for seed in EVALUATION_SEEDS:
        for spec in STRATEGY_SPECS:
            ...
            del surrogate
        prefix.configure_reproducibility(seed, deterministic=True)
        surrogate, plan, masks = closure.initialize_blackbox(
            victim,
            official_weight,
            seed,
        )
    ...
    for seed in EVALUATION_SEEDS:
        reused_results.append(
            reuse_result(
                source_by_key[(seed, closure.CANDIDATE_DROP06_CASE)],
                local_case=BASE_CASE,
                origin="reused_lab06_candidate",
        ...
        reused_results.append(
            reuse_result(
                ...
                origin="reused_lab06_matched_blackbox",
    ...
    payload = {
        "schema_version": 3,
        "experiment": "08_structure_conv1_dependency",
        "protocol": "MS",
        **prefix.protocol_metadata(first_query),
        ...
        "base_case": {
            "case": BASE_CASE,
            "source_case": closure.CANDIDATE_DROP06_CASE,
        ...
        "source": {
            "lab06_candidate": str(source_path.relative_to(ROOT)),
