...
def build_conv2_states(victim):
    ...
    if not set(CONV1_WEIGHTS) <= set(base_states):
        raise RuntimeError("Lab08 基础集合不再包含固定五个 conv1。")
...
def source_rows(source_by_key):
    ...
    for seed in SEEDS:
        ...
        base["origin"] = "reused_lab06_conv1_candidate"
        base["source_case"] = closure.CANDIDATE_DROP06_CASE
        conv1[seed] = base

        boundary = copy.deepcopy(source_by_key[(seed, BLACKBOX_CASE)])
        boundary["origin"] = "reused_lab06_matched_blackbox"
...
def trained_result(
    *,
    ...
    return {
        "seed": seed,
        "case": CONV2_CASE,
        "origin": "trained_lab08_conv2_swap",
...
def plot_result(path: Path, result_by_key, aggregate):
    ...
    figure.suptitle("Lab08: replace protected conv1 with matched conv2", y=1.03)
    figure.tight_layout()
...
def main() -> int:
    ...
    source_path = (
        ROOT / "results" / "lab" / "06_weight" / "candidate.json"
    )
    victim_checkpoint = (
    ...
    out_dir = ROOT / "results" / "lab" / "08_structure"
        ...
        **prefix.protocol_metadata(first_query),
        ...
        "protection_sets": {
            ...
            "shared": {
                ...
                "bn_gamma": list(closure.derive_bn_gamma(victim)),
            },
        },
        "source": {
    ...
    print("[OK] 写入 Lab08 conv1/conv2 直接替换结果。")
    return 0
